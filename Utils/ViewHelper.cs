...
using static DrugScanner.Server.Utils.ViewHelper;
...
public static class ViewHelper
{
  ...
  {
    return value.Equals("desc", StringComparison.CurrentCultureIgnoreCase) || value.IsTruthy();
  }
    ...
    Dictionary<string, string> searchColumnsOptions, string defaultColumn = "")
  {
    ...
    columns = columns.Where(column => !string.IsNullOrWhiteSpace(column)).ToList();

    values ??= new List<string>();
    values = values.Where(value => !string.IsNullOrWhiteSpace(value)).ToList();

    operators ??= new List<string>();
    operators = operators.Where(_operator => !string.IsNullOrWhiteSpace(_operator)).ToList();

    stack ??= new List<string>();
    stack = stack.Where(value => !string.IsNullOrWhiteSpace(value)).ToList();
  ...
  public class GetParams
  {
    ...
    /// <summary>
    /// Flag to determine is pagination should be applied
    /// </summary>
    ...
    /// </summary>
    public List<string> SearchOperators { get; set; } = [];
    /// <summary>
    /// The type of stacking in the filters applied to the search query: "or" or "and"
    /// </summary>
    public List<string> SearchStack { get; set; } = [];
  }

  public class GetOptions
  {
    public bool PreventOperation { get; set; }
  }
...
public static partial class ViewHelperExtensions
{
  ...
  public static Expression<Func<T, bool>>? GetExpression<T>(this List<SearchRule> rules)
  {
    ...
    for (int i = 0; i < rules.Count; i++)
    {
      ...
      expr = (expr == null || i == 0) ? e1 : (rules[i - 1].Stack == SearchStackType.Or ? Expression.Or(expr, e1) : Expression.AndAlso(expr, e1));
    }
      ...
      case RegExOperators.IsAnyOf:
        ...
        if (left.Type == typeof(string))
        {
          ...
          expr = Expression.Call(
            ...
            typeof(List<string>).GetMethod("Contains", [typeof(string)])!,
            left
          );
        }
        else
